...
def Model3_deepXplore_drop(input_shape=None,
                     drop_rate=0.4, nb_classes=10, kernel_size=(5,5)):
    # block1
    model = Sequential()
    model.add(Convolution2D(32, kernel_size, activation='relu', padding='same', name='block1_conv1', input_shape=input_shape)) #1
    model.add(MaxPooling2D(pool_size=(2, 2), name='block1_pool1')) #2
    model.add(Lambda(lambda x: K.dropout(x, drop_rate)))
