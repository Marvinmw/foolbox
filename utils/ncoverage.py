...
class NCoverage():
    ...
    def KMNC(self, input_x, K=1000,onlyKMNC=True, **kwargs):
        ...
        for layer_name in self.layer_to_compute:
            layer_model = Model(inputs=self.model.inputs,
                                outputs=self.model.get_layer(layer_name).output)
            layer_outputs = layer_model.predict(input_x)
            for neron_idx in range(layer_outputs.shape[-1]):
                    total += 1
                    ...
                    #hist, _ = np.histogram(d, bins=K, range=(low, high))
    ...
    def diffScore(self, input_data):
        ...
        for layer_name in self.layer_to_compute:
            ...
            diff += np.count_nonzero((coveredneurons - self.covered_neruons[layer_name])>0)  # neurons
            del layer_model
            del layer_outputs
            del scaled
            del nc_covered
