...
import exp_models.cifar.Dense_Net as densenet
import exp_models.cifar.Network_in_Network as netinnet
import exp_models.cifar.ResNet as resnet
...
def get_coverage(k, score):
    ...
    buckets = np.digitize(normalized, np.linspace(0, 1, k))
...
def computeCor(x,y):
    kendallscore = kendalltau(x, y)[0]
    cor = distcorr(x, y)
    pcor = np.corrcoef(x, y)[0][1]
    return np.round(kendallscore, 4), np.round(cor,4), np.round(pcor,4)
...
def load_model(model_name, weight_path, inputshape, num_classes,isDrop=False, droprate=0.3):
    ...
    if model_name == 'dense_net':
        ...
        output = densenet.densenet(img_input, num_classes, drop=isDrop, droprate=droprate)
    ...
    if model_name == 'resnet':
        img_input = Input(shape=inputshape)
        output = resnet.residual_network(img_input, classes_num=num_classes,drop=isDrop, droprate=droprate)
        model = Model(img_input, output)
        sgd = optimizers.SGD(lr=.1, momentum=0.9, nesterov=True)
        model.compile(loss='categorical_crossentropy', optimizer=sgd, metrics=['accuracy'])
        model.load_weights(weight_path,by_name=True)
